 ...
 * 新版-点击流统计-页面pv和显示次数
 * wpid 主图书三级分类，没有为空串''
 * psku 主图书sku，没有为空串''，根据它来判断此图书为pop还是非pop
 * markId 推荐位标记，找张斌要
 * op s:显示、p:pv
 */
...
var noBookOver = {
    ...
    init: function( type ) {
        ...
        this.outOfStockTPL = ''
            ...
            +'        </div>'
            +'        <div class="mc">'
            +'            <div id="nobook-related-list">'
            +'                <div class="nobook-related-list">'
            +'                    <a href="javascript:;" class="spec-control disabled" id="nobook-forward"></a>'
            +'                    <a href="javascript:;" class="spec-control" id="nobook-backward"></a>'
            +'                    <div id="nobook-list">'
        ...
        var isNoBook = $('#product-intro').hasClass('product-intro-nobook'),
            isOver = $('#product-intro').hasClass('product-intro-bookover');

        if ( !isNoBook && !isOver ) {

            $('#out-of-stock,#nobook-related-list,#bookover-related-list,#bookover1-related-list').remove();
            return false;
        }
        if ( isNoBook ) {
            this.noBook();
    ...
    noBook: function( isBookOver ) {
//        var imgStr = isBookOver ? '<img height="100" width="100" src="${pageConfig.FN_GetImageDomain(list.skuId)}n4/${list.imgUrl}">' : '<img height="130" width="130" src="${pageConfig.FN_GetImageDomain(list.skuId)}n3/${list.imgUrl}">';
//        // 无货推荐列表[老服务]
//        var noBook_TPL_OLD = '{for list in html}'
...
//
//                    if ( isBookOver ) {
//                        if ( $('#nobook-related-list').length < 1 ) {
//                            $('#choose').after('<div id="nobook-related-list"><p>其它类似图书</p><div class="nobook-related-list"><div class="iloading">正在加载中，请稍候...</div></div>');
//                        } else {
//                            $('#bookover-related-list').show();
//                        }
//                    } else {
...
//
//
//                    $('#nobook-related-list .nobook-related-list').html( '<a href="javascript:;" class="spec-control disabled" id="nobook-forward"></a><a href="javascript:;" class="spec-control" id="nobook-backward"></a><div id="nobook-list"><ul>' + noBook_TPL_OLD.process( data ) + '</ul></div>' );
//                    $('#nobook-related-list').attr('loaded', 'true');
//
//
//                    //图片滚动【无货页面】
//                    $('#nobook-list').imgScroll({
//                        visible: scrollVisible_bookover,
//                        showControl: true,
//                        speed: 200,
//                        step: scrollVisible_bookover,
//                        loop: false,
//                        prev: '#nobook-forward',
//                        next: '#nobook-backward',
//                        disableClass: 'disabled'
//                    });
//
//                } else if ( !data || data.html == null || data.html.length < 1 ) {
//                    _this.noBookNoData(isBookOver);
//                }
//            }
//        });
    },
    bookOver: function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#bookover1-related-list').show();
                $('#bookover1-related-list .bookover1-related-list').html( '<a href="javascript:;" class="spec-control disabled" id="bookover1-forward"></a><a href="javascript:;" class="spec-control disabled" id="bookover1-backward"></a><div id="bookover1-list"><ul>' + resHTML.join('') + '</ul></div>' );

                getPriceNum(pageConfig.product.listBrosweBroswe, $('#bookover1-related-list'));

                //图片滚动【下柜页面-看了还买了】
                $('#bookover1-list').imgScroll({
                    visible: scrollVisible_bookover,
                    showControl: false,
                    speed: 200,
                    step: scrollVisible_bookover,
                    loop: false,
                    prev: '#bookover1-forward',
                    next: '#bookover1-backward',
                    disableClass: 'disabled',
                    width: 130,
                    height:165
                });
    ...
    noBookNoData: function( isBookOver ) {
        var _this = this;
        ...
        if (!isBookOver) {
            setTimeout(function() {
				...
				var rec_103000 = new Grecommend(G.sku, rid, readCookie('ipLoc-djd'), $('#out-of-stock #nobook-list'), 20);
        ...
        var imgStr = isBookOver ? '<img height="100" width="100" src="${pageConfig.FN_GetImageDomain(list.wid)}n4/${list.imgurl}">' : '<img height="130" width="130" src="${pageConfig.FN_GetImageDomain(list.wid)}n3/${list.imgurl}">';
        // 无货推荐列表[新服务]
        var noBook_TPL = '{for list in MySoldOut}'
            +'<li data-push="${pageConfig.product.noBookData.push(list.wid)}" class="fore fore${list_index}" onclick="reClick2(\''+pageConfig.product.cat[0]+'&SORec\','+pageConfig.product.skuid+', \'${list.wid}#${list.wmeprice}\', ${list_index})">'
            +'  <div class="p-img">'
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if ( data.MySoldOut !== null && data !== null ) {
                    ...
                    $('#nobook-related-list').attr('iplocation', getCookie("ipLocation"));

                    if ( isBookOver ) {
                        $('#bookover-related-list').show().find('.bookover-related-list').html('<a href="javascript:;" class="spec-control disabled" id="bookover-forward"></a><a href="javascript:;" class="spec-control disabled" id="bookover-backward"></a><div id="bookover-list"><ul>' + noBook_TPL.process(data) + '</ul></div>');

                        getPriceNum(pageConfig.product.noBookData, $('#bookover-related-list'));

                        //图片滚动【下柜页面-其它类似图书】
                        $('#bookover-list').imgScroll({
                            visible: scrollVisible_bookover,
                            showControl: true,
                            speed: 200,
                            step: scrollVisible_bookover,
                            loop: false,
                            prev: '#bookover-forward',
                            next: '#bookover-backward',
                            disableClass: 'disabled',
                            width: 130,
                            height:165
                        });
                        ...
                        $('#nobook-related-list .nobook-related-list').html( '<a href="javascript:;" class="spec-control disabled" id="nobook-forward"></a><a href="javascript:;" class="spec-control" id="nobook-backward"></a><div id="nobook-list"><ul>' + noBook_TPL.process(data) + '</ul></div>' );


                        getPriceNum(pageConfig.product.noBookData, $('#nobook-related-list .nobook-related-list'));

                        //图片滚动【无货页面】
                        $('#nobook-list').imgScroll({
                            visible: scrollVisible_nobook,
                            showControl: true,
                            speed: 200,
                            step: scrollVisible_nobook,
                            loop: false,
                            prev: '#nobook-forward',
                            next: '#nobook-backward',
                            disableClass: 'disabled'
...
if(warestatus!=1&&warestatus!=2){
    $("#product-intro").attr("class","product-intro-bookover");
    $('#out-of-stock,#nobook-related-list').remove();
}
$.pbuyurl="";
...
function flashPurchaseChange1(){
	...
	if($('#summary-countdown').length==0){
		$.ajax({
			url: 'http://red.jd.com/book/remainTime.html?skuid=' + pageConfig.product.skuid,
			dataType: 'jsonp',
			success: function(r) {
				if(r){
...
//                }
//                if(r.type!=4){
//                    if($("#product-intro .bookover-title button").length==0&&$("#product-intro .bookover-title h3").length>0)$("<button type='button' clstag='shangpin|keycount|product|bbtn'>进入卖家店铺</button>").appendTo("#product-intro .bookover-title h3");
//                    $("#product-intro .bookover-title button").unbind("click").click(function(){window.location=$.getShopUrl(r);});
//                }   
...
function getStockDescWords(state,isPurchase,skuid,skukey,arrivalDate,isNotice,ext,provinceId,rn,pr){
    ...
    if (state == 33){
        ...
        pageConfig.product.havestock = false;
        $('body').addClass('no-stock');
        text = "<strong>无货</strong>，此图书暂时售完"+(isNotice?"，<a href='#none' id='notify-stock' target='_blank'>到货通知</a>":"");
        if (skuid&&skuid.length == 8 && !isPurchase){
            text = "<strong>无货</strong>，此图书不再销售，欢迎选购其它图书";
        }
    }
        ...
        text = "<strong>预订</strong>，"+(arrivalDate?"预计"+arrivalDate+"日后有货，现在可下单":"图书到货后发货，现在可下单")+"<span></span>";
    }
    text += yfInfo;
    return text;
}
...
function chooseType() {
    var shoppingselect = $('#choose-type .book'),
        amount = $('#choose-amount'),
        buyLink = $('#choose-btn-append .btn-append'),
        selectBook = $('#choose-type .selected').eq(0); 
    if ( !selectBook.attr('data') ) {
        return false;
    }       
    ...
    if ( selectBook.length > 0 ) {
        var data = selectBook.attr('data').split('|'),
            newlink = data[1].replace(/wid=\d{6,}/, 'wid=' + pageConfig.product.skuid );
        buyLink.attr('href', newlink);
    }
    if ( shoppingselect.length == 1 && selectBook.length < 1 ) {
...
//		}
//        if(window.noBookOver)noBookOver.init();
//        return;
//    }
...
//    $("#product-intro").attr("class","product-intro-nobook");
//    if($("#choose-noresult").length==0&&areaSurportDelive&&pageConfig.product.skuid!=938747&&pageConfig.product.skuid!=938749){$("<li id='choose-noresult'><div class='dd'><strong>所选地区该图书暂时无货，非常抱歉！</strong></div></li>").insertAfter("#choose-result");}
//    else if(!areaSurportDelive){$("#choose-noresult").remove();}
//    $.easybuy_button.hide();
//    $.divide_button.hide();
//    if($.append_button.length>0){$.append_button.show();if($.append_button.attr("href")!="#none"){$.pbuyurl=$.append_button.attr("href")}$.append_button.attr("href","#none").attr("onclick","").attr("title","图书已无货").unbind("click"); }//购物车
...
//        }
//    }
//    if(window.noBookOver)noBookOver.init(pageConfig.product.type);
};
//stock callback
...
function getProvinceStockCallback(result,choosetype) {
    ...
    if (result.stock) {     
        ...
        $("#store-prompt").html(getStockDescWords(result.stock.code==2?-1:result.stock.StockState,true,pageConfig.product.skuid,pageConfig.product.skuidkey,result.stock.ArrivalDate,pageConfig.product.isNotice,result.stock.Ext,currentAreaInfo.currentProvinceId,result.stock.rn,result.stock.pr)
            +(pageConfig.product.yfinfo.nofree?"，<span style='cursor:pointer' title='一个店铺购买多件图书，只收取一次运费'>运费：<span style='color:#f00;'>￥"+pageConfig.product.yfinfo.cash+"</span><span>":""));
    ...
    if (pageConfig.product.skuid>1000000000){
            if(!$._ptload){
                ...
                window._showPopTemplete=function(r){
                    if(result.stock.StockState==36){
                        if(r&&r.reserveDeliveryDay){
                            ...
                            $("#store-prompt").html(stockdesc+"，"+"此图书为预订图书，下单后在"+$._ptloadcon);
                        }
                    }
                ...
                if($._ptloadcon)$("#store-prompt").html(stockdesc+"，"+"此图书为预订图书，下单后在"+$._ptloadcon);
            }
    }
}
/**
...
function getChildAreaHtml(fid,level){
    ...
    if (idName){
        $("#stock_province_book,#stock_city_book,#stock_area_book,#stock_town_book").hide();
...
function getIdNameByLevel(level){
    var idName = "";
    if (level == 1){
        idName = "stock_province_book";
    }
    else if (level == 2){
        idName = "stock_city_book";
    }
    else if (level == 3){
        idName = "stock_area_book";
    }
    else if (level == 4){
        idName = "stock_town_book";
    }
    return idName;
}
//需要的地址层级
...
function newEasyBuyInit(){
	if(dCashDescInfo.loadPriceCnt==dCashDescInfo.loadStockCnt){
		if(bookEasyBuy.bigger10&&pageConfig.product.isStock&&!pageConfig.product.YuShou
    ...
    if (r&&r.length>0){
        if (new Number(r[0].p)>0){  
            ...
            if(new Number(r[0].p)<10){
                bookEasyBuy.bigger10 = false;
            }else{
                bookEasyBuy.bigger10 = true;
			}
    ...
    if(r&&r.resultCode==1&&r.skuId&&r.skuId!=pageConfig.product.skuid+''){
		...
		if((r.skuId+"").length==10){
			if($("#local-seller").length==0){
				$('<div class="mt" id="local-seller"><span class="fl"><b>本地在售卖家</b></span></div><div class="mc" id="local-seller1"><ul><li id="J_'+r.skuId+'"><div class="fl"><a target="_blank" href="http://book.jd.com/'
			...
			$('<div class="mt" id="jd-seller"><span class="fl"><b>其它在售卖家</b></span></div><div class="mc" id="jd-seller1"><ul><li id="J_'+r.skuId+'"><div class="fl"><a target="_blank" href="http://book.jd.com/'
...
function GetCurrentStock(){
	if($("#choose-type .book").length>0){	
		...
		if($("#choose-type .book").length>1||pageConfig.product.stockExtraParamChange){
...
var provinceHtml = '<div data-widget="tabs" class="m JD-stock" id="JD-stock">'
                                +'<div class="mt">'
                                +'    <ul class="tab">'
                                +'        <li data-index="0" data-widget="tab-book" class="curr"><a href="#none" class="hover"><em>请选择</em><i></i></a></li>'
                                +'        <li data-index="1" data-widget="tab-book" style="display:none;"><a href="#none" class=""><em>请选择</em><i></i></a></li>'
                                +'        <li data-index="2" data-widget="tab-book" style="display:none;"><a href="#none" class=""><em>请选择</em><i></i></a></li>'
                                +'        <li data-index="3" data-widget="tab-book" style="display:none;"><a href="#none" class=""><em>请选择</em><i></i></a></li>'
                                +'    </ul>'
                                +'    <div class="stock-line"></div>'
                                +'</div>'
                                +'<div class="mc" data-area="0" data-widget="tab-content" id="stock_province_book">'
                                +'    <ul class="area-list">'
                                ...
                                +'    </ul>'
                                +'</div>'
                                +'<div class="mc" data-area="1" data-widget="tab-content" id="stock_city_book"></div>'
                                +'<div class="mc" data-area="2" data-widget="tab-content" id="stock_area_book"></div>'
                                +'<div class="mc" data-area="3" data-widget="tab-content" id="stock_town_book"></div>'
                                +'</div>';
...
(function(){
    if($(".product-intro-bookover").length>0){
        if(window.noBookOver)noBookOver.init(pageConfig.product.type);
        return;
    }
    CurrentAreaInfoInit();
...
//		}
    ...
    $("#stock_province_book a").unbind("click").click(function() {
        currentPageLoad.isLoad = false;
        resetBindMouseEvent();
        try{
            CurrentAreaInfoInit();
...
var choose_color=$("#choose-color a");var choose_colori=$("#choose-color .book");
var choose_version=$("#choose-version .book");
...
var alert_choose="所选"+alert_choose_color.replace("选择","")+"该"+alert_choose_version.replace("选择","")+"图书在该地区无货";
var alert_choose1="所选"+alert_choose_version.replace("选择","")+"该"+alert_choose_color.replace("选择","")+"图书在该地区无货";
var csobj={};var scobj={};
...
if(choose_color.length>0||choose_version.length>0){
    ...
    changeColorSize=function(flag){
        ...
        if(cur_color&&csobj[cur_color]){
            ...
            for(var i=0,j=choose_version.length;i<j;i++){
                ...
                if(!(version&&csobj[cur_color][version])
                    ...
                    $(choose_version[i]).attr("class","book disabled");
                    $(choose_version[i]).find("b").hide();
                    choose_version_obj.css("cursor","not-allowed").attr("title",alert_choose);
                }
                else{
                    if($(choose_version[i]).attr("class")=="book disabled")$(choose_version[i]).attr("class","book");
        ...
        if(cur_version&&scobj[cur_version]){
            ...
            for(var i=0,j=choose_colori.length;i<j;i++){
                ...
                if(!(color&&scobj[cur_version][color])
                    ...
                    $(choose_colori[i]).attr("class","book disabled");
                    $(choose_colori[i]).find("b").hide();
					if(only_one_size){choose_color_obj.css("cursor","not-allowed").attr("title",alert_choose);}
                }
                else{
                    if($(choose_colori[i]).attr("class")=="book disabled")$(choose_colori[i]).attr("class","book");
                    $(choose_colori[i]).find("b").show();
                }
            }
        }
        else{
            $("#choose-color .disabled").attr("class","book");
        }
		...
		$("#choose-result").show().find(".dd").html("<em>已选择</em>"+(cur_color&&cur_color!=" "?"<strong>“"+cur_color+"”</strong>":"")+(cur_color&&cur_color!=" "&&cur_version&&cur_version!=" "?"，":"")+(cur_version&&cur_version!=" "?"<strong>“"+cur_version+"”</strong>":"")
			+(cur_color?"":"<span class='book-warnning'><s></s>请"+alert_choose_color+"</span>")+(cur_version?"":"<span class='book-warnning'><s></s>请"+alert_choose_version+"</span>"));
		if(!cur_color){
			$("#choose-color").addClass("book-hl-bg");
		}
		else{
			$("#choose-color").removeClass("book-hl-bg");
		}
		if(!cur_version){
			$("#choose-version").addClass("book-hl-bg");
		}
		else{
			$("#choose-version").removeClass("book-hl-bg");
		}
        if(!flag){
            if($("#choose-noresult").length>0){$("#choose-noresult").remove();}
            $.easybuy_button.hide();
    ...
    $("#choose-color a").attr("href","#none").unbind("click").click(function(){
        if($($(this).parent()).attr("class")=="book disabled"&&only_one_size)return;
		cur_same_colorsize_stock.choose=1;
        $("#choose-color .selected").attr("class","book");
        $($(this).parent()).attr("class","book selected");
    ...
    $("#choose-version a").attr("href","#none").unbind("click").click(function(){
        if($($(this).parent()).attr("class")=="book disabled")return;
		cur_same_colorsize_stock.choose=1;
        var s=$(this).text();
...
(function(){
    if(!window.bookShopJsona&&!window.bookShopJsonb)return;
    var shopTempArray=[];
    var leftDom = $("#related-sorts").parent();
    if(window.bookShopJsona&&bookShopJsona.searchLink){
        var shopSearchHtml='<div id="sp-search" class="m m2" clstag="shangpin|keycount|product|pop-03">'
                    +'<div class="mt"><h2>店内搜索</h2></div>'
                    +'<div class="mc">'
                        +'<p class="sp-form-book1"><label for="sp-keyword">关键字：</label><span><input type="text" id="sp-keyword"/ onkeydown="javascript:if(event.keyCode==13){pageConfig.searchClick(1);}"></span></p>'
                        +'<p class="sp-form-book2"><label for="sp-price">价&#x3000;格：</label><span><input type="text" id="sp-price" onkeyup="changeSpPrice(\'sp-price\');" onkeydown="javascript:if(event.keyCode==13){pageConfig.searchClick(1);}"/> 到 <input type="text" id="sp-price1" onkeyup="changeSpPrice(\'sp-price1\');" onkeydown="javascript:if(event.keyCode==13){pageConfig.searchClick(1);}"/></span></p>'
                        +'<p class="sp-form-book3"><label for="">&#x3000;&#x3000;&#x3000;</label><span><input type="submit" value="搜索" id="btnShopSearch"/></span></p>'
                    +'</div>'
                +'</div>';
        shopTempArray.push(shopSearchHtml);
    }
    if(window.bookShopJsonb&&bookShopJsonb.shopCategory&&bookShopJsonb.shopCategory.length>0){
        var shopSortsHtml='<div id="sp-category" class="m m2" clstag="shangpin|keycount|product|pop-04">'
            ...
            +'</div>';
        var categoryContent=[];
        for (var i=0,j=bookShopJsonb.shopCategory.length;i<j;i++){
            categoryContent.push("<dl><dt"+((bookShopJsonb.shopCategory[i].childs&&bookShopJsonb.shopCategory[i].childs.length>0)?"":" class='sp-single'")
                +"><a href='"+(bookShopJsonb.shopCategory[i].url?bookShopJsonb.shopCategory[i].url:"#none")+"' target='_blank'><s></s>"+bookShopJsonb.shopCategory[i].title+"</a></dt>");
            if(bookShopJsonb.shopCategory[i].childs&&bookShopJsonb.shopCategory[i].childs.length>0){
                for (var t=0,h=bookShopJsonb.shopCategory[i].childs.length;t<h;t++){
                    categoryContent.push("<dd><a href='"+(bookShopJsonb.shopCategory[i].childs[t].url?bookShopJsonb.shopCategory[i].childs[t].url:"#none")+"' target='_blank'>"+bookShopJsonb.shopCategory[i].childs[t].title+"</a></dd>");
                }
            }
    ...
    if(window.bookShopJsonb&&bookShopJsonb.hotSale&&bookShopJsonb.hotSale.length==5){
        var shopHotSaleHtml='<div id="sp-hot-sale" class="m m2 m3" clstag="shangpin|keycount|product|pop-05">'
                ...
                +'</div>';
        var hotSaleContent=[];
        for (var i=0,j=bookShopJsonb.hotSale.length;i<j;i++){
            hotSaleContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://book.jd.com/'+bookShopJsonb.hotSale[i].skuId+'.html" title="'+bookShopJsonb.hotSale[i].goodsName+'" target="_blank"><img alt="'+bookShopJsonb.hotSale[i].goodsName+'" data-lazyload="'+getImgFullPath(bookShopJsonb.hotSale[i].skuId,bookShopJsonb.hotSale[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://book.jd.com/'+bookShopJsonb.hotSale[i].skuId+'.html" target="_blank" title="'+bookShopJsonb.hotSale[i].goodsName+'">'+bookShopJsonb.hotSale[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
                                +'<div class="p-count fl"><s>'+(i+1)+'</s><b>热销'+bookShopJsonb.hotSale[i].saleCount+'件</b></div>'
                                +'<div class="p-price fr"><strong p="J_'+bookShopJsonb.hotSale[i].skuId+'"></strong></div>'
                            +'</div></li>');
        }
        shopTempArray.push(shopHotSaleHtml.replace("{content}",hotSaleContent.join("")));
    }
    if(window.bookShopJsonb&&bookShopJsonb.hotFocus&&bookShopJsonb.hotFocus.length==5){
        var shopHotFocusHtml='<div id="sp-hot-fo" class="m m2 m3" clstag="shangpin|keycount|product|pop-06">'
                    +'<div class="mt"><h2>热门关注</h2></div>'
                    +'<div class="mc"><ul>{content}</ul></div>'
                +'</div>';
        var hotFocusContent=[];
        for (var i=0,j=bookShopJsonb.hotFocus.length;i<j;i++){
            hotFocusContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://book.jd.com/'+bookShopJsonb.hotFocus[i].skuId+'.html" title="'+bookShopJsonb.hotFocus[i].goodsName+'" target="_blank"><img alt="'+bookShopJsonb.hotFocus[i].goodsName+'" data-lazyload="'+getImgFullPath(bookShopJsonb.hotFocus[i].skuId,bookShopJsonb.hotFocus[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://book.jd.com/'+bookShopJsonb.hotFocus[i].skuId+'.html" target="_blank" title="'+bookShopJsonb.hotFocus[i].goodsName+'">'+bookShopJsonb.hotFocus[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
                                +'<div class="p-count fl"><s>'+(i+1)+'</s><b>'+bookShopJsonb.hotFocus[i].focusCount+'人关注</b></div>'
                                +'<div class="p-price fr"><strong p="J_'+bookShopJsonb.hotFocus[i].skuId+'"></strong></div>'
                            +'</div></li>');
        }
        shopTempArray.push(shopHotFocusHtml.replace("{content}",hotFocusContent.join("")));
    }
    if(window.bookShopJsona&&bookShopJsona.shopRec&&bookShopJsona.shopRec.length>0){
        var shopRecommHtml='<div id="sp-reco" class="m m2 m3" clstag="shangpin|keycount|product|pop-07">'
                    +'<div class="mt"><h2>店长推荐</h2></div>'
                    +'<div class="mc"><ul>{content}</ul></div>'
                +'</div>';
        var shopRecContent=[];
        for (var i=0,j=bookShopJsona.shopRec.length;i<j;i++){
            shopRecContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://book.jd.com/'+bookShopJsona.shopRec[i].skuId+'.html" title="'+bookShopJsona.shopRec[i].goodsName+'" target="_blank"><img alt="'+bookShopJsona.shopRec[i].goodsName+'" data-lazyload="'+getImgFullPath(bookShopJsona.shopRec[i].skuId,bookShopJsona.shopRec[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://book.jd.com/'+bookShopJsona.shopRec[i].skuId+'.html" target="_blank" title="'+bookShopJsona.shopRec[i].goodsName+'">'+bookShopJsona.shopRec[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
                               // +'<div class="p-count fl"></div>'
                                +'<div class="p-price"><strong p="J_'+bookShopJsona.shopRec[i].skuId+'"></strong></div>'
                            +'</div></li>');
        }
        shopTempArray.push(shopRecommHtml.replace("{content}",shopRecContent.join("")));
    }
    if(window.bookShopJsona&&bookShopJsona.shopAd){
        shopTempArray.push('<div id="sp-ad" class="m" clstag="shangpin|keycount|product|pop-08"><a href="'+bookShopJsona.shopAd.adUrl+'" target="_blank" title="'+bookShopJsona.shopAd.title+'"><img data-lazyload="'+bookShopJsona.shopAd.adImg+'" alt="'+bookShopJsona.shopAd.title+'"/></a></div>');
    }
    if(shopTempArray.length>0){
        ...
        pageConfig.searchClick=function(type){
            ...
            if (keyword||minprice||maxprice){
                window.open(bookShopJsona.searchLink+"?orderBy=5&keyword="+keyword+"&minPrice="+minprice+"&maxPrice="+maxprice,"_blank");
            }
            else{
                if(type == 1) return;
                window.open(bookShopJsona.searchLink+"?orderBy=5","_blank");
            }
        };
...
var turl=$("#product-detail-5 .book-detail a:last").attr("href"); 
