...
function setAccordion(t) {
    ...
    6 == parseInt($.browser.version, 10) ? "left" : "margin-left", e.removeClass("advanced"), e.find("a").bind("mouseenter", function() {
        $(this).stop(!0).animate({opacity: i}, 800).siblings("a").stop(!0).animate({opacity: a}, 800).end().find("img").stop(!0).animate({left: s}, 400), $(this).parent(".book").siblings().find("a").each(function() {
...
function setShowcase(t) {
    ...
    if (pageConfig.wideVersion && pageConfig.compatible, $.browser.msie && 9 >= parseInt($.browser.version, 10)) {
        var i = e.find(".sc-list").find(".book");
...
BackPanelNew.prototype = {init: function(t) {
        ...
        i = this.wrap.find(".bp-book").eq(0).outerWidth(), a = parseInt(this.wrap.css("marginLeft")), this.resizeMask || (this.mask.css({height: this.wrap.outerHeight() - 10,width: this.wrap.outerWidth()}), this.resizeMask = !0), i > (t - e) / 2 && (this.resized = !0)
    },add: function(t) {
    	...
    	// 去掉右侧工具条
    	return ;
        var e = this, i = '<div class="{if newTag}bp-book-new{/if} bp-book bp-book-${cName}" data-top="${scrollTo}">    <a href="${href}" class="${cName}" target="${target}">${text}</a><s></s></div>', a = $(i.process(t));
...
var Floors = {cacheData: null,data: function(t) {
        ...
        this.skus = [];
        var e = '<ul class="lh">    {for book in data}    {if Number(book.jp)>=0}    <li data-push="${Grecommend.skus.push(book.sku)}" class="fore1" onclick="reClick2012(\'cai2012\',\'${book.sku}#${book.jp}\',0)" data-clk="${book.clk}">        <div class="p-img ld"><a target="_blank" href="http://my.jd.com/product/likes.html?id=${book.sku}"><img height="130" width="130" alt="${book.t}" data-img="1" src="${pageConfig.FN_GetImageDomain(book.sku)}n3/${book.img}"></a></div>        <div class="p-name"><a target="_blank" href="http://my.jd.com/product/likes.html?id=${book.sku}" title="${book.t}">${book.t}</a></div>        <div class="p-price">\u4eac\u4e1c\u4ef7\uff1a<strong class="J-p-${book.sku}">\uffe5${book.jp}</strong></div>    </li>    {/if}    {/for}</ul>';
...
(function() {
    ...
    var t = $('#virtuals div[data-widget="tab-book"]').index($("#virtuals .curr")) || 0;
    $("#virtuals").Jtab({type: "dynamic",index: t,source: "data-url",delay: 200}, function(t, e, i) {
    ...
    }), pageConfig.TPL_MSlide = {booksWrap: '<ul class="slide-books">{innerHTML}</ul>',booksContent: '{for book in json}            {var v=pageConfig.FN_GetCompatibleData(book)}            <li class="hide" clstag="homepage|keycount|home2013|09a${parseInt(book_index)+2}"><a href="${v.href}" target="_blank" title="${v.alt}">                <img height="${v.height}" width="${v.width}" src="${v.src}" data-img="2" />            </a></li>        {/for}',controlsWrap: '<div class="slide-controls">{innerHTML}</div>',controlsContent: '{for book in json}            <span class="{if parseInt(book_index)==0}curr{/if}">${parseInt(book_index)+1}</span>        {/for}'}, pageConfig.TPL_FSlide = {booksWrap: '<div class="slide-bookswrap"><div class="slide-books">{innerHTML}</div></div>',booksContent: '{for book in json}            <div><a href="${book.href}" target="_blank"><img src="${book.src}" width="${book.width}" height="${book.height}" alt="${book.alt}" data-img="2"></a></div>        {/for}',controlsWrap: '<div class="slide-controls">{innerHTML}</div>',controlsContent: '{for book in json}            <span class="{if parseInt(book_index)==0}curr{/if}"><b></b></span>        {/for}'}, pageConfig.FN_SetPrice = function(t) {
        var e = t.split(".");
    ...
    }, pageConfig.TPL_Tabs = '<ul class="lh style3">        {for book in data}        {var x=pageConfig.FN_SetPrice(book.c)}        {var y=pageConfig.FN_SetImg(book.a,book.d)}        <li class="fore${parseInt(book_index)}">            <div class="p-img ld"><a target="_blank" href="${pageConfig.FN_GetUrl(book.f,book.a)}" title="${book.b}">{if book.e}${pageConfig.FN_SetPromotion(book.e)}{/if}<img height="100" width="100" alt="${book.b}" data-img="1" src="${y}"></a></div>            <div class="p-name"><a target="_blank" href="${pageConfig.FN_GetUrl(book.f,book.a)}" title="${book.b}">${book.b}</a></div>            <div class="p-price"><span>\uffe5</span>${x}</div>        </li>        {/for}    </ul>', pageConfig.TPL_Tabs_jp = '<ul class="lh style3">        {for book in data}        <li class="fore${parseInt(book_index)}">            <div class="p-img ld"><a target="_blank" href="${pageConfig.FN_GetUrl(book.f,book.a)}" title="${book.b}">{if book.e}${pageConfig.FN_SetPromotion(book.e)}{/if}<img height="100" width="100" alt="${book.b}" data-img="1" src="${pageConfig.FN_GetImageDomain(book.a)}${book.d}"></a></div>            <div class="p-name" style="height:1.5em;"><a target="_blank" href="${pageConfig.FN_GetUrl(book.f,book.a)}" title="${book.b}">${book.b}</a></div>            <div class="p-price"><span>${book.c}</span></div>        </li>        {/for}    </ul>', pageConfig.FN_Tabs = function(t, e) {
        var i = t.split(",");
        obj = {}, html = "";
        var a;
        ...
        e.find(".b2").animate({height: "34px"})
    }), pageConfig.accordionData !== void 0 ? ($("#clothing").addClass("clothing-new"), renderAccordion(pageConfig.accordionData)) : (pageConfig.TIMER_Clothing = null, $("#clothing .accordion .book").mouseover(function() {
        var t = $(this);
    ...
    function t() {
        var t = $("#floors-list [data-fid]"), e = $("#backpanel-floor"), a = e.find(".bp-book-floor-links");
