...
function clothingAbTest() {
    if ("home" == pageConfig.navId) {
        ...
        if (a) {
            var b = getHashProbability(a, 1e4), c = $("#_JD_ALLSORT div.fore6"), d = c.find(".subbook .fore1 dt a"), e = c.find(".subbook .fore2 dt a");
...
!function() {
    ...
    a.fn.dropdown = function(b, c) {
        var b = a.extend({className: "book",current: "hover",enterDelay: 10,leaveDelay: 300,onmouseleave: null}, b);
        a.each(this, function() {
            function d() {
                function a(a, b) {
    ...
    a.fn.Jtab = function(b, c) {
        if (this.length) {
            "function" == typeof b && (c = b, b = {});
            var d = a.extend({type: "static",auto: !1,event: "mouseover",currClass: "curr",source: "data-tag",hookKey: "data-widget",hookBookVal: "tab-book",hookContentVal: "tab-content",stay: 5e3,delay: 100,threshold: null,mainTimer: null,subTimer: null,index: 0,compatible: !1}, b || {}), e = a(this).find("*[" + d.hookKey + "=" + d.hookBookVal + "]"), f = a(this).find("*[" + d.hookKey + "=" + d.hookContentVal + "]"), g = d.source.toLowerCase().match(/http:\/\/|\d|\.aspx|\.ascx|\.asp|\.php|\.html\.htm|.shtml|.js/g);
        ...
        if (this.length) {
            ...
            var d = a.extend({auto: !1,reInit: !1,data: [],defaultIndex: 0,slideWidth: 0,slideHeight: 0,slideDirection: 1,speed: "normal",stay: 5e3,delay: 150,maxAmount: null,template: null,showControls: !0}, b || {}), e = a(this), f = null, g = null, h = null, i = null, j = null, k = function() {
                ...
                if (d.reInit) {
                    ...
                    a.json = a.json.slice(1), i = b.booksContent.process(a), e.find(".slide-books").eq(0).append(i), e.find(".slide-controls").eq(0).html(j)
                } else {
                    var k = b.booksWrap.replace("{innerHTML}", b.booksContent) + b.controlsWrap.replace("{innerHTML}", b.controlsContent), m = k.process(a);
                    e.html(m)
                }
                f = e.find(".slide-books"), g = e.find(".slide-controls"), h = g.find("span"), l(), n(), c && c(e)
            }, l = function() {
    ...
    return b = jQuery.extend({books_per_page: 10,num_display_entries: 10,current_page: 0,num_edge_entries: 0,link_to: "#",prev_text: "Prev",next_text: "Next",ellipse_text: "...",prev_show_always: !0,next_show_always: !0,callback: function() {
            return !1
        }}, b || {}), this.each(function() {
        function c() {
            return Math.ceil(a / b.books_per_page)
        }
        function d() {
        ...
        a = !a || 0 > a ? 1 : a, b.books_per_page = !b.books_per_page || b.books_per_page < 0 ? 1 : b.books_per_page;
        var h = jQuery(this);
    ...
    a.jdCalcul = function(b) {
        var c = null, b = b.join(","), d = "http://qiang.jd.com/HomePageNewLimitBuy.ashx?callback=?&ids=" + b, e = "http://book.jd.com/", f = function(b) {
...
var category = {OBJ: $("#_JD_ALLSORT"),
		//URL_Serv: "http://manage.taotao.com/web/bookcat/all?callback=category.getDataService",
		URL_Serv: "http://127.0.0.1:8081/rest/bookcat/all?callback=category.getDataService",
      ...
      //  $.getJSONP(this.URL_BrandsServ, category.getBrandService)
        ...
        return d
    },renderBook: function(a) {
        var b = '<div class="book fore${index+1}">    <span data-split="1" {if pageConfig.isHome}clstag="homepage|keycount|home2013|0${601+parseInt(index)}a"{/if}><h3>${n}</h3><s></s></span>    <div class="i-mc">        <div onclick="$(this).parent().parent().removeClass(\'hover\')" class="close">\xd7</div>        <div class="subbook" {if pageConfig.isHome}clstag="homepage|keycount|home2013|0${601+parseInt(index)}b"{/if}>            {for subbook in i}            <dl class="fore${parseInt(subbook_index)+1}">                <dt>${category.FN_GetLink(1,subbook)}</dt>                <dd>{for link in subbook.i}<em>${category.FN_GetLink(2,link)}</em>{/for}</dd>            </dl>            {/for} </div> </div></div>', c = '<div class="book book-col2 fore${index+1}">    <span data-split="1" {if pageConfig.isHome}clstag="homepage|keycount|home2013|0${601+parseInt(index)}a"{/if}><h3>${n}</h3><s></s></span>    <div class="i-mc">        <ul class="hide title-list lh">            <li class="fore1"><a href="http://chongzhi.jd.com/">\u5145\u503c\u7f34\u8d39</a></li>            <li class="fore2"><a href="http://caipiao.jd.com/">\u4eac\u4e1c\u5f69\u7968</a></li>            <li class="fore3"><a href="http://channel.jd.com/4938-12316.html">\u57f9\u8bad\u6559\u80b2</a></li>            <li class="fore4"><a href="http://jipiao.jd.com/ticketquery/flightHotcity.action">\u4f18\u9009\u673a\u7968</a></li>            <li class="fore5"><a href="http://channel.jd.com/4938-12300.html">\u65c5\u884c\u7b7e\u8bc1</a></li>            <li class="fore6"><a href="http://game.jd.com/">\u4eac\u4e1c\u6e38\u620f</a></li>        </ul>        <div onclick="$(this).parent().parent().removeClass(\'hover\')" class="close">\xd7</div>        <div class="subbook" {if pageConfig.isHome}clstag="homepage|keycount|home2013|0${601+parseInt(index)}b"{/if}>            {for book in i}            <div class="sub-book-col sub-book-col${book_index} fl">                {for subbook in book.tabs}                <dl class="fore${parseInt(subbook_index)+1}">                    <dt>${category.FN_GetLink(1,subbook)}</dt>                    <dd>{for link in subbook.i}<em>${category.FN_GetLink(2,link)}</em>{/for}</dd>                </dl>                {/for}					{if book_index==1}<div class="cat-right-con" id="JD_sort_${t}" clstag="homepage|keycount|home2013|0614c"><div class="loading-style1"><b></b>\u52a0\u8f7d\u4e2d\uff0c\u8bf7\u7a0d\u5019...</div></div>{/if}            </div>            {/for}        </div>    </div></div>';
      ...
      //  $.getJSONP(this.URL_BrandsServ, category.getBrandService)
        ...
        $.each(a.data, function(a) {
            this.index = a, "l" == this.t && (this.i = c.FN_RefactorJSON(this.i, 7)), b.push(c.renderBook(this, a))
        });
        ...
        $("#_JD_ALLSORT").dropdown({delay: 0,onmouseleave: function() {
                $("#_JD_ALLSORT .book").removeClass("hover")
            }}, function(a) {
        ...
        var b = "", c = "", d = "", e = "";
        return b = this.FN_SetLink(a), c = '{if p.length!=0}<dl class="categorys-promotions">    <dd>        <ul>            {for book in p}            <li>                <a href="${book.u}" target="_blank">                {if book.i}                    <img src="${book.i}" width="194" height="70" title="${book.n}" style="margin-bottom: 4px;" />                {else}                    ${book.n}                {/if}                </a>            </li>            {/for}        </ul>    </dd></dl>{/if}', d = '{if b.length!=0}<dl class="categorys-brands">    {if id=="k"}        <dt>\u63a8\u8350\u54c1\u724c\u51fa\u7248\u5546/\u4e66\u5e97</dt>    {else}        {if id=="l"}        <dt>\u63a8\u8350\u4ea7\u54c1</dt>        {else}        <dt>\u63a8\u8350\u54c1\u724c</dt>        {/if}    {/if}    <dd>        <ul>            {for book in b} <li><a href="${book.u}" target="_blank">${book.n}</a></li> {/for}        </ul>    </dd></dl>{/if}', e = /c|b|d/.test(a) ? b + d + c : b + c + d
...
!function() {
    ...
    })), document.getElementById("navbooks") ? $("#navbooks li").Jdropdown() : $("#navbooks-2013 li").Jdropdown()
...
//    }, a.prototype.selectBookNode = function(a) {
...
//                    return function(c) {
//                        a.iLastModified > b || (a.iLastModified = b, a.onloadBooks(c))
//                    }
//                }((new Date).getTime())})
//        }, 150)
...
//        }
//    }, a.prototype.onloadBooks = function(a) {
//        if (0 == a.length)
...
//                        continue;
//                    var z = "book1", A = b.replace(g, {id: o,title: s.keyword,cid: s.cid,cLevel: s.level,className: z,cname: s.cname,amount: s.amount,suggest_pos: j - 1});
...
pageConfig.FN_InitSidebar = function() {
    ...
    this.scroll = function() {
        ...
        this.addBook("<a href='#' class='gotop' title='\u4f7f\u7528\u5feb\u6377\u952eT\u4e5f\u53ef\u8fd4\u56de\u9876\u90e8\u54e6\uff01'><b></b>\u8fd4\u56de\u9876\u90e8</a>")
    }
